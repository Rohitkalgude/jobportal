...
function AllJob() {
  ...
  const handleApply = async (jobId, jobTitle, companyName, designation) => {
    try {
      ...
      if (
        ...
        toast.error("Incomplete profile. Please update your details.");
        return;
      }
      ...
      localApplications.push(applicantData);
      localStorage.setItem("applicationData", JSON.stringify(localApplications));
  ...
  const handleSaveJob = async (jobId) => {
    try {
      ...
      const errMsg =
        error.response?.data?.message || "Failed to save the job.";
      console.error("Save Job Error:", errMsg);
      toast.error(errMsg);
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredJobs.length > 0 ? (
              filteredJobs.map((job) => (
                <div
                  key={job._id}
                  className="bg-white p-6 rounded-lg shadow-lg border border-gray-200 hover:scale-[1.02] transition-transform hover:shadow-2xl"
                >
                  ...
                  <div className="flex justify-between gap-4 mt-6">
                    <button
                      ...
                      className="w-1/2 bg-gray-700 text-white py-2 rounded-md hover:bg-gray-800 transition"
                    >
                      Apply
                    </button>
